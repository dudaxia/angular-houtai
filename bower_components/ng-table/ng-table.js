(function(angular, factory) {
    ...
    app.factory('ngTableParams', ['$q', '$log', function($q, $log) {
        ...
        var ngTableParams = function(baseParameters, baseSettings) {
            ...
            this.odataOrderBy = function() {
                var sorting = [];
                for (var column in params.sorting) {
                    sorting.push(column + ' ' + params.sorting[column]);
                }
                return sorting.join(',');
            };
            ...
            this.reload = function() {
                ...
                this.checkboxes.keys = [];
                this.checkboxes.rows = [];
                if (settings.groupBy) {
            ...
            return {
                ...
                compile: function(element, attrs) {
                    ...
                    if (attrs.checkboxShow) {
                        row.html('<td class="with-checkbox" ><input type="checkbox"  ng-model="params.checkboxes.rows[$index]"/> </td>' + row.html())
                    };
                    angular.forEach(row2.find('td'), function(item) {
                        var el = angular.element(item);
                        if (!el.hasClass('with-checkbox')) {
                    ...
                    return function(scope, element, attrs) {
                        ...
                        if (attrs.checkboxShow) {
                            ...
                            scope.checkAll = function(e) {
                                angular.forEach(scope.params.data, function(item, $index) {
                                    scope.params.checkboxes.rows[$index] = e.target.checked;
                                });
                            };
                            scope.$watch('params.checkboxes.rows', function(rows) {
                                if (!scope.params.data.length) {
                                    return;
                                }
                                var checked = 0,
                                    total = scope.params.data.length;
                                scope.params.checkboxes.items = [];
                                scope.params.checkboxes.keys = [];
                                angular.forEach(scope.params.data, function(item, $index) {
                                    var flag = scope.params.checkboxes.rows[$index];
                                    checked += flag || 0;
                                    if (flag) {
                                        scope.params.checkboxes.items.push(item);
                                        if (attrs.checkboxKey) {
                                            scope.params.checkboxes.keys.push(item["" + attrs.checkboxKey + ""]);
                                        };
                                    };
                                });
                                scope.params.checkboxes.checked = (checked == total);
                            }, true);
                        }
                        ...
                        if (!element.hasClass('ng-table')) {
                            ...
                            if (attrs.pagerTarget) {
                                // $(attrs.pagerTarget).append(paginationTemplate)
                                $(attrs.pagerTarget).html(paginationTemplate)
                            } else {
    ...
    angular.module('ngTable').run(['$templateCache', function($templateCache) {
        ...
        $templateCache.put('ng-table/header.html', '<tr> <th class="with-checkbox" ng-if="checkboxShow"> <input type="checkbox" ng-model="params.checkboxes.checked" ng-click="checkAll($event)" ng-disabled="params.settings().$loading"/> </th> <th ng-repeat="column in $columns" ng-class="{ \'th-sortable\': parse(column.sortable) }" ng-click="sortBy(column, $event)" ng-show="column.show(this)" ng-init="template=column.headerTemplateURL(this)" class="{{column.class}}"> <div ng-if="!template" ng-show="!template">{{parse(column.title)}}<span class="th-sort" ng-if="parse(column.sortable)"> <i class="fa fa-sort-down " ng-class="params.sorting()[parse(column.sortable)]!=\'desc\'?\'text-active\':\'text\'"></i> <i class="fa fa-sort-up" ng-class="params.sorting()[parse(column.sortable)]!=\'asc\'?\'text-active\':\'text\'"></i> <i class="fa fa-sort"></i> </span></div> <div ng-if="template" ng-show="template"> <div ng-include="template"></div> </div> </th> </tr> <tr ng-show="show_filter" class="ng-table-filters"> <th ng-repeat="column in $columns" ng-show="column.show(this)" class="filter"> <div ng-repeat="(name, filter) in column.filter"> <div ng-if="column.filterTemplateURL" ng-show="column.filterTemplateURL"> <div ng-include="column.filterTemplateURL"></div> </div> <div ng-if="!column.filterTemplateURL" ng-show="!column.filterTemplateURL"> <div ng-include="\'ng-table/filters/\' + filter + \'.html\'"></div> </div> </div> </th> </tr> ');
        $templateCache.put('ng-table/pager.html', '<div class="ng-cloak ng-table-pager"> <ul class="pagination ng-table-pagination m-t-sm m-b-none"> <li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="prev" ng-click="params.page(page.number)" href="">&laquo;</a> <a ng-switch-when="first" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="">&raquo;</a> </li> </ul> </div> ');
    }]);
    return app;
}));
