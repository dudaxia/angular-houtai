    ...
    this.pop = function (type, title, body, timeout, bodyOutputType, clickHandler) {
        if (angular.isObject(type)) {
            ...
            this.toast = {
                ...
                clickHandler1:params.clickHandler1
            };
    ...
    return {
        ...
        link: function (scope, elm, attrs) {
            ...
            $rootScope.$on('toaster-newToast', function () {
                scope.config = angular.extend({}, scope.config, toaster.options); 
        ...
        controller: ['$scope', '$element', '$attrs', function ($scope, $element, $attrs) {
            ...
            $scope.click1 = function (toaster, isCloseButton) {
                var removeToast = true;
                if (toaster.clickHandler1) {
                    toaster.clickHandler1();
                }
                if (removeToast) {
                    $scope.removeToast(toaster.id);
                }
        ...
        '<div  id="toast-container" ng-class="[config.position, config.animation]">' +
            '<div ng-repeat="toaster in toasters" class="toast" ng-class="toaster.type" ng-click="click(toaster)" ng-mouseover="stopTimer(toaster)"  ng-mouseout="restartTimer(toaster)">' +
              ...
              '<div ng-class="config.message" ng-switch on="toaster.bodyOutputType">' +
                '<div ng-switch-when="trustedHtml" ng-bind-html="toaster.html" ng-click="click1(toaster)"></div>' +
                '<div ng-switch-when="template" ng-click="click1(toaster)"><div ng-include="toaster.bodyTemplate"></div></div>' +
                '<div ng-switch-default ng-click="click1(toaster)">{{toaster.body}}</div>' +
              '</div>' +
            '</div>' +
        '</div>'
    };
