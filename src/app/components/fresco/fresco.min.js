angular.module('ngTicket').directive('frescoImg', [function() {
    return {
        restrict: 'AE',
        templateUrl: '/app/components/fresco/fresco.img.html',
        scope: {
            list: '='
        },
        link: function(scope, iElement, iAttrs) {
            if (!String.IsNullOrEmpty(scope.list)) {
                if (angular.isString(scope.list)) {
                    scope.list = JSON.parse(scope.list)
                };
                var imglist = $.map(scope.list, function(item, index) {
                    return {
                        options: {
                            thumbnail: item
                        },
...
}]).directive('frescoImgs', [function() {
    return {
        restrict: 'AE',
        templateUrl: '/app/components/fresco/fresco.imgs.html',
        scope: {
            list: '='
        },
        link: function(scope, iElement, iAttrs) {
...
}]).directive('frescoImgText', [function() {
    return {
        restrict: 'AE',
        templateUrl: '/app/components/fresco/fresco.imgtext.html',
        scope: {
            list: '='
        },
        link: function(scope, iElement, iAttrs) {
...
window.Fresco = (function($) {
    ...
    var C = {
        within: function(a) {
            ...
            if (!b.bounds) {
                b.bounds = $.extend({}, L._boxDimensions)
            }
            var c = b.bounds,
                size = $.extend({}, a),
                f = 1,
        ...
        bind: function(e) {
            ...
            function touchStart(c) {
                var d = $(this),
                    ...
                    touches = c.originalEvent.touches,
                    touchEvent = (touches && touches[0]) || c,
                    pageX = touchEvent.pageX,
                    pageY = touchEvent.pageY,
                    moveX = touchEvent.pageX,
                    moveY = touchEvent.pageY,
                ...
                function touchMove(a) {
                    ...
                    touchEvent = a.originalEvent.touches ? a.originalEvent.touches[0] : a;
                    newTime = new Date().getTime();
                    newPageX = touchEvent.pageX;
                    newPageY = touchEvent.pageY;
                    xDiff = newPageX - moveX;
                    yDiff = newPageY - moveY;
                    var b = new Date().getTime();
                    if (supress && ((f.suppresX && Math.abs(xDiff) < f.scrollSupressionThreshold) || (f.suppresY && Math.abs(yDiff) < f.scrollSupressionThreshold) || (time && b - time < 100))) {
                        return
                    }
                    if (initialMove) {
                        initialMove = false;
                ...
                d.bind(G._events.touchmove, touchMove).one(G._events.touchend, touchEnd)
            }
            $(e).data("fr-touchstart", touchStart);
            $(e).bind(G._events.touchstart, touchStart)
        },
        unbind: function(c) {
            var d = {
                start: 0,
            ...
            $.each(d, function(a, b) {
                ...
                if (d[a]) {
                    $(c).unbind(G._events["touch" + a], d[a]).removeData("fr-touch" + a)
                }
            })
        }
    };
    ...
    var H = {
        get: function(c, d, e) {
            ...
            if (!f) {
                var g;
                if ((g = H.preloaded.get(c)) && g.dimensions) {
                    f = g;
                    H.cache.set(c, g.dimensions, g.data)
                }
            }
            if (!f) {
                if (d.track) {
                ...
                switch (type) {
                        ...
                        h.onload = function() {
                            ...
                            if (d.dragImage) {
                                createDragImage(h, function(a, b) {
                                    data.dragImage = a;
                                    data.dragScale = b;
                                    H.cache.set(c, f.dimensions, data);
                                    if (d.track) {
                                        H.loading.clear(c)
                                    }
                                    if (e) {
                    ...
                    case "vimeo":
                        ...
                        var j = $.getJSON(protocol + "//vimeo.com/api/oembed.json?url=" + protocol + "//vimeo.com/" + i + "&callback=?", $.proxy(function(a) {
                            ...
                            H.cache.set(c, b.dimensions, data);
                            if (d.track) {
                                H.loading.clear(c)
                            }
                            if (e) {
    ...
    H.loading = new H.Loading();
    H.preload = function(c, d, e) {
        if ($.type(d) == "function") {
            e = d;
            d = {}
        }
        ...
        if (d.once && H.preloaded.get(c)) {
            return
        }
        var f;
        if ((f = H.preloaded.get(c)) && f.dimensions) {
            if ($.type(e) == "function") {
                e($.extend({}, f.dimensions), f.data)
            }
            return
        ...
        H.preloaded.cache.add(g);
        image.src = c
    };
    H.preloaded = {
        get: function(a) {
            return H.preloaded.cache.get(a)
        },
    ...
    var I = (function() {
        ...
        function create(d, e, f) {
            d = d || {};
            f = f || {};
            d.skin = d.skin || (t.skins[J.defaultSkin] ? J.defaultSkin : "fresco");
            if (u.IE && u.IE < 7) {
                d.skin = "IE6"
            }
            var g = d.skin ? $.extend({}, t.skins[d.skin] || t.skins[J.defaultSkin]) : {},
    ...
    $.extend(Loading.prototype, {
        ...
        start: function(a) {
            this.center();
            var b = L._frames && L._frames[L._position - 1];
            this.element.stop(1, 0).fadeTo(b ? b.view.options.effects.loading.show : 0, 1, a)
        },
        stop: function(a, b) {
            var c = L._frames && L._frames[L._position - 1];
    ...
    var J = {
        ...
        build: function() {
            ...
            $(document.body).prepend(this.element = $("<div>").addClass("fr-window").append(this.bubble = $("<div>").addClass("fr-bubble").hide().append(this.frames = $("<div>").addClass("fr-frames").append(this.move = $("<div>").addClass("fr-frames-move"))).append(this.thumbnails = $("<div>").addClass("fr-thumbnails")).append(this.touchMenu = $("<div>").addClass("fr-touch-menu")).append(this.touchCaption = $("<div>").addClass("fr-touch-caption"))));
        ...
        _hide: function() {
            ...
            K.disable();
            M.block();
            var a = L._frames && L._frames[L._position - 1];
    ...
    var K = {
        ...
        onkeydown: function(a) {
            if (!this.enabled || !J.element.is(":visible")) {
                return
            }
    ...
    var L = {
        ...
        startObserving: function() {
            $(window).bind("resize", $.proxy(function() {
                if (J.states.get("visible") && !J._pinchZoomed) {
                    this.resize();
                    this.updateMove()
                }
            }, this));
        ...
        previous: function(a) {
            ...
            if (a || b) {
                J.setPosition(this.getSurroundingIndexes().previous)
            } else {
                var c;
                if (!b && (c = L._frames && L._frames[L._position - 1]) && c.view.options.ui == "touch") {
                    this.moveTo(this._position)
                }
            }
        },
        ...
        next: function(a) {
            var b = this.mayNext();
            if (a || b) {
                J.setPosition(this.getSurroundingIndexes().next)
            } else {
                var c;
                if (!b && (c = L._frames && L._frames[L._position - 1]) && c.view.options.ui == "touch") {
                    this.moveTo(this._position)
        ...
        setXY: function(a) {
            ...
            if (M.visible() && M._vars.orientation == "vertical") {
                a.x -= M._vars.thumbnails.width
            }
            var b = {
        ...
        resize: function() {
            ...
            if (this._frames[0] && this._frames[0].view.options.ui == "touch") {
                $.each(this._frames, function(i, a) {
                    a.frame.css({
                        width: L._touchWidth + "px"
                    })
                });
                this.move.css({
                    width: L._touchWidth * this._frames.length + "px"
                })
            }
        },
        updateDimensions: function(e) {
            ...
            if (M.visible()) {
                M.updateVars();
                var g = M._vars.orientation == "horizontal",
                    subtract = g ? "height" : "width",
                    subtractPx = M._vars.thumbnails[subtract],
                    offset = {
                        left: g ? 0 : subtractPx
                    };
            ...
            if (N.visible()) {
                N.updateVars();
                f.height -= (N._vars.menu.height + N._vars.caption.height);
                if (this._noCaptions) {
                    f.height += N._vars.caption.height
                }
            }
            var h = $.extend({}, f);
            this._sideWidth = 0;
            switch (ui) {
                ...
                case "touch":
                    ...
                    var k = this._frames && this._frames[0].frame,
                        originalWidth = false;
                    var l = this.move.attr("style");
                    this.move.removeAttr("style");
                    var m, landscape, width, touchPercentage;
                    this.frames.css(px({
                        height: h.height
                    }));
                    $.each(this._frames, $.proxy(function(i, F) {
                        var a = F.frame;
                        if (!a.data("portrait")) {
                            var b = this._frames.length == 1 ? {
                                portrait: 1,
                                landscape: 1
                            } : F.view.options.touch.width;
                            a.data("portrait", Math.max(b.portrait, 0.5)).data("landscape", Math.max(b.landscape, 0.5))
                        } else {
                            m = Math.floor(f.width * a.data("portrait"));
                            landscape = Math.floor(f.width * a.data("landscape"))
                        }
                    }, this));
                    this.setOrientation(j);
                    this._touchWidth = j == "portrait" ? m : landscape;
                    $.extend(h, {
                        width: this._touchWidth || 0
                    });
                    this.move.attr("style", l);
                    break
            }
            this._dimensions = f;
        ...
        show: function(b, c) {
            ...
            M[f.view.options.thumbnails ? "show" : "hide"]();
            N[f.view.options.ui == "touch" ? "show" : "hide"]();
    ...
    $.extend(Frame.prototype, {
        ...
        remove: function() {
            this.clearUITimer();
            if (this._track) {
                L.removeTracking(this._position);
                this._track = false
            }
            this._removeVideo();
            this._reset();
        ...
        build: function() {
            var b = this.view.options.ui,
                positions = J.views.length;
            L.move.append(this.frame = $("<div>").addClass("fr-frame").append(this.box = $("<div>").addClass("fr-box").addClass("fr-box-has-ui-" + b).addClass("fr-box-has-type-" + this.view.type)));
            ...
            if (b == "touch") {
                ...
                if (this.view.type == "image" && ((c == "next" && (this.view.options.loop || (!this.view.options.loop && this._position != J.views.length))) || c == "close")) {
        ...
        _getInfoHeight: function(a) {
            ...
            if (this.view.options.ui == "outside") {
                a = Math.min(a, L._boxDimensions.width)
            }
        ...
        _whileVisible: function(b, c) {
            var d = [];
            var e = J.element.add(J.bubble).add(this.frame).add(this.ui);
            if (c) {
                e = e.add(c)
            }
            $.each(e, function(i, a) {
        ...
        getLayout: function() {
            ...
            if (ui == "outside") {
                ...
                this._whileVisible($.proxy(function() {
                    ...
                    while ((a < attempts)) {
                        ...
                        var b = 0.5 * (L._boxDimensions.height - 2 * border - (i.vertical ? i.vertical * 2 : 0) - f.height);
                        if (b < h.height) {
                            f = C.within(f, {
                                bounds: $.extend({}, {
                                    width: f.width,
            ...
            if (ui == "outside") {
                ...
                h.width = Math.min(f.width, L._boxDimensions.width)
            }
            var g = $.extend({}, L._boxDimensions);
            if (ui == "outside") {
                p.box = {
                    dimensions: {
                        width: L._boxDimensions.width
                    },
                    position: {
                        left: 0.5 * (L._dimensions.width - L._boxDimensions.width)
                    }
                }
            }
            p.ui = {
        ...
        load: function(p, q) {
            ...
            if (!q && !(H.cache.get(this.view.url) || H.preloaded.getDimensions(this.view.url))) {
                J.loading.start()
            }
            this._loading = true;
            this._loadTimer = setTimeout($.proxy(function() {
                ...
                switch (this.view.type) {
                    case "image":
                        ...
                        H.get(this.view.url, {
                            ...
                            if (this.view.options.ui == "outside" && ((l = this.view.options.onClick) && l == "next" || l == "previous-next")) {
                                ...
                                if ((this._position != L._frames.length) || m) {
                            ...
                            this.frame.find(".fr-download-image").each($.proxy(function(i, d) {
                                var e = $("<img>").addClass("fr-download-image").attr({
                                    ...
                                    side = $(d).data("side");
                                if (u.IE && u.IE < 9) {
                                    var f = parseInt(J.element.css("z-index")) || 0;
                                    e.css({
                                        "z-index": f
                                    });
                                    $(d).parents().css({
                    ...
                    case "vimeo":
                        ...
                        H.get(this.view.url, $.proxy(function(a, b) {
                            if (!this.view) {
                                return
                            }
                            var c = o.width,
        ...
        afterLoad: function(a) {
            ...
            var c;
            if (L._frames && (c = L._frames[L._position - 1]) && (c.view.url == this.view.url || c.view.options.ui == "touch")) {
                J.loading.stop()
            }
            if (a) {
                a()
        ...
        resize: function() {
            if (this.content) {
                ...
                L[(this._track ? "set" : "remove") + "Tracking"](this._position);
            ...
            switch (f) {
                case "outside":
                    ...
                    if (this.view.caption) {
                        ...
                        if (h > L._boxDimensions.height - g.info.height) {
                            h = L._boxDimensions.height - g.info.height
                        }
                        var i = L._sideWidth + c.left + g.wrapper.margin.left + this._border;
                        if (i < L._sideWidth) {
                            i = L._sideWidth
                        }
                        if (i + g.info.width > L._sideWidth + g.box.dimensions.width) {
                            i = L._sideWidth
                        }
                        this.info.css({
                            top: h + "px",
    ...
    $.extend(View.prototype, {
        initialize: function(a) {
            ...
            a.options = I.create(a.options, a.type, a._data);
            $.extend(this, a);
            return this
        }
    });
    var M = {
        ...
        resize: function() {
            ...
            if (u.IE && u.IE < 9) {
                J.timeouts.clear("ie-resizing-thumbnails");
                J.timeouts.set("ie-resizing-thumbnails", $.proxy(function() {
                    this.updateVars();
            ...
            this.wrapper.css(d);
            this._previous.css(px(b.sides.previous));
            this._previous.css(px(b.sides.next));
            if (this._position) {
        ...
        moveTo: function(a, b) {
            ...
            var f = L._frames && L._frames[L._position - 1];
            var g = {},
                animateCSS = {};
    ...
    $.extend(Thumbnail.prototype, {
        ...
        load: function() {
            if (this._loaded || this._loading || !M.visible() || this._removed) {
                return
            }
        ...
        resize: function() {
            ...
            this.thumbnail_frame.css(px(M._vars.thumbnail_frame));
            var a = M._vars.orientation == "horizontal";
            this.thumbnail_frame.css(px({
                top: a ? 0 : M._vars.thumbnail_frame.height * (this._position - 1),
                left: a ? M._vars.thumbnail_frame.width * (this._position - 1) : 0
            }));
            if (!this.thumbnail_wrapper) {
                return
            }
            var b = M._vars.thumbnail;
    ...
    var N = {
        ...
        startObserving: function() {
            ...
            this.touchMenu.add(this.touchCaption).bind("touchmove", $.proxy(function(a) {
        ...
        updateVars: function() {
            ...
            var h = this.touchMenu.innerHeight(),
                c_height = this.touchCaption.innerHeight();
            if (g) {
                this.setOverflowClass(true)
            }
            vars.menu.height = h;
            vars.caption.height = c_height;
            if (!g) {
                this.setOverflowClass(true)
            }
        ...
        load: function(b) {
            ...
            this.touchCaption[(!c ? "add" : "remove") + "Class"]("fr-touch-caption-nocaptions");
            this._noCaptions = !c
        },
        clear: function() {
            this._views = [];
        ...
        expand: function(a) {
            ...
            var b = B.viewport();
            var c = -1 * Math.min(b.height, (this._vars.caption.overflowHeight || 0));
            if (this._vars.caption.overflowHeight > b.height) {
                this.info.css({
                    height: b.height + "px"
                }).addClass("fr-touch-caption-overflow-scroll");
    ...
    var O = {
        ...
        show: function(c) {
            if (this._disabled) {
                this.showFallback.apply(O, v.call(arguments));
                return
            }
            var d = arguments[1] || {},
    ...
    $.extend(t, {
        show: function(a) {
            O.show.apply(O, v.call(arguments));
            return this
        },
        hide: function() {
            J.hide();
